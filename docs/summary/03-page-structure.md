...
| `/`（index） | `routes/news` | HomeLayout | `NewsList` + Twitter 关注 |
| `news` | `routes/news` | HomeLayout | 同上 |
...
- `ReportSelecter`（`src/interface/ReportSelecter.tsx`）—— 首页/导航栏的"粘贴报告链接"输入，导出 `constructURL`。
