...
MONK_DIR = 'src/analysis/retail/monk'
...
def main():
    monk_issues = {}
    for root, dirs, files in os.walk(MONK_DIR):
        for file in files:
            if file.endswith(('.tsx', '.ts')) and not file.endswith('.test.tsx') and not file.endswith('.test.ts'):
                ...
                if untranslated:
                    monk_issues[filepath] = []
                    for item_type, text, pos in untranslated:
                        line = get_line_number(filepath, pos)
                        monk_issues[filepath].append({
                            'line': line,
                            'type': item_type,
                            'text': text
                        })
                        
    for f, items in sorted(monk_issues.items()):
        print(f"File: {f}")
        for item in items:
