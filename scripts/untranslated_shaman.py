...
SHAMAN_DIR = 'src/analysis/retail/shaman'
...
def main():
    shaman_issues = {}
    for root, dirs, files in os.walk(SHAMAN_DIR):
        for file in files:
            if file.endswith(('.tsx', '.ts')) and not file.endswith('.test.tsx') and not file.endswith('.test.ts'):
                ...
                if untranslated:
                    shaman_issues[filepath] = []
                    for item_type, text, pos in untranslated:
                        line = get_line_number(filepath, pos)
                        shaman_issues[filepath].append({
                            'line': line,
                            'type': item_type,
                            'text': text
                        })
                        
    for f, items in sorted(shaman_issues.items()):
        print(f"File: {f}")
        for item in items:
