...
export default function Guide({ modules, events, info }: GuideProps<typeof CombatLogParser>) {
  return (
    <>
      ...
      <Section
        title={t({ id: 'paladin.holy.section.healingCooldowns', message: 'Healing cooldowns' })}
      >
      ...
      <SubSection title={t({ id: 'paladin.holy.subsection.holyPower', message: 'Holy Power' })}>
        <p>
          <>{t({ id: 'paladin.holy.holyPower.description.p1', message: 'Since ' })}
            ...
            {t({ id: 'paladin.holy.holyPower.description.p2', message: 'spenders are so impactful,' })}
            {' '}
            {t({ id: 'paladin.holy.holyPower.description.p3', message: 'minimizing waste should be a priority.' })}
            {' '}
            ...
            {' '}
            {t({ id: 'paladin.holy.holyPower.description.p4', message: 'is often the most reliable choice when deciding which' })}
            {' '}
            <ResourceLink id={RESOURCE_TYPES.HOLY_POWER.id} />
            {t({ id: 'paladin.holy.holyPower.description.p5', message: 'spender to use. As a general rule of thumb, if casting' })}
            {' '}
            {info.combatant.hasTalent(talents.ETERNAL_FLAME_TALENT) ? (
              <SpellLink spell={talents.ETERNAL_FLAME_TALENT} />
            ) : (
            ...
            {' '}
            {t({ id: 'paladin.holy.holyPower.description.p6', message: 'won\'t result in significant overhealing, it\'s usually the best option. This is because' })}
            {' '}
            <SpellLink spell={talents.LIGHT_OF_DAWN_TALENT} />
            {t({ id: 'paladin.holy.holyPower.description.p7', message: 'tends to overheal and targets allies randomly, making it less effective.' })}
          </>
        </p>
        <p>
