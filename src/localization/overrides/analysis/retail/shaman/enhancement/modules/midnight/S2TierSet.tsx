  ...
  private onCast(event: CastEvent) {
    ...
    if (this.hasVoltaicBlaze && this.deps.spellUsable.isAvailable(SPELLS.VOLTAIC_BLAZE_CAST.id)) {
      addInefficientCastReason(
        event,
        <>
          <SpellLink spell={SPELLS.VOLTAIC_BLAZE_CAST} />
          {t({
            id: 'shaman.enhancement.s2TierSet.voltaicBlaze.inefficient',
            message: ' was available and is a higher priority cast.',
          })}
        </>,
      );
    ...
    if (
      ...
      addInefficientCastReason(
        ...
        <>
          ...
          {t({
            id: 'shaman.enhancement.s2TierSet.crashLightning.inefficient',
            message: ' was available and is a higher priority cast.',
          })}
        </>,
      );
    ...
    return (
      <StartAlignedRoundedPanel key={spell.id}>
        <strong>
          ...
          {t({ id: 'shaman.enhancement.s2TierSet.holdTime.label', message: ' Hold Time' })}
        </strong>
        <div style={{ display: 'flex', justifyContent: 'space-evenly', fontSize: '20px' }}>
          {summary && (
            <>
              <div>
                {formatDurationMillisMinSec(summary.average, 1)}{' '}
                <small>
                  {t({ id: 'shaman.enhancement.s2TierSet.holdTime.average', message: 'average' })}
                </small>
              </div>
              <div>
                {formatDurationMillisMinSec(summary.minimum, 1)}{' '}
                <small>
                  {t({ id: 'shaman.enhancement.s2TierSet.holdTime.shortest', message: 'shortest' })}
                </small>
              </div>
              <div>
                {formatDurationMillisMinSec(summary.maximum, 1)}{' '}
                <small>
                  {t({ id: 'shaman.enhancement.s2TierSet.holdTime.longest', message: 'longest' })}
                </small>
              </div>
            </>
          )}
          <div>
            ...
            <small>
              {t({
                id: 'shaman.enhancement.s2TierSet.holdTime.castsDelayed',
                message: 'casts delayed',
              })}
            </small>
          </div>
          {additionalStat}
        ...
        {!summary && (
          <small>
            {t({
              id: 'shaman.enhancement.s2TierSet.holdTime.none.p1',
              message: 'No hold time recorded for ',
            })}
            <SpellLink spell={spell} />
            {t({ id: 'shaman.enhancement.s2TierSet.holdTime.none.p2', message: '.' })}
          </small>
        )}
  ...
  private description(): JSX.Element {
    ...
    return (
      <>
        <p>
          {t({
            id: 'shaman.enhancement.s2TierSet.description.p1',
            message: 'The Season 2 tier set significantly increases the priority of ',
          })}
          {vb}
          {t({ id: 'shaman.enhancement.s2TierSet.description.p2', message: ' and ' })}
          {cl}
          {t({
            id: 'shaman.enhancement.s2TierSet.description.p3',
            message: ', they should be cast as soon as they are available.',
          })}
        </p>
        <p>
          {t({
            id: 'shaman.enhancement.s2TierSet.description2.p1',
            message: 'Even short delays before casting ',
          })}
          {cl}
          {t({
            id: 'shaman.enhancement.s2TierSet.description2.p2',
            message: ' can quickly add up to a large number of lost casts over a fight. Holding ',
          })}
          {vb}
          {t({
            id: 'shaman.enhancement.s2TierSet.description2.p3',
            message: ' can similarly lead to many missed ',
          })}
          {cl}
          {t({
            id: 'shaman.enhancement.s2TierSet.description2.p4',
            message:
              " opportunities due to the 2-piece bonus's cooldown reduction from ",
          })}
          <SpellLink spell={TALENTS.FIRE_NOVA_TALENT} />
          {t({ id: 'shaman.enhancement.s2TierSet.description2.p5', message: '.' })}
        </p>
      </>
    );
  }
      ...
      >
        <ItemSetBonuses setId={SHAMAN_MID2_ID} title={MID2_SET_TITLE}>
          <ItemSetBonus
            ...
            label={
              <>
                <SpellLink spell={SPELLS.FIRE_NOVA_DAMAGE} />
                {t({
                  id: 'shaman.enhancement.s2TierSet.statistic.fireNovaBonus',
                  message: ' bonus',
                })}
              </>
            }
              ...
              <ItemSetBonus
                ...
                label={
                  <>
                    <SpellLink spell={TALENTS.CRASH_LIGHTNING_TALENT} />
                    {t({
                      id: 'shaman.enhancement.s2TierSet.statistic.crashLightningBonus',
                      message: ' bonus',
                    })}
                  </>
                }
              >
                <UptimeIcon /> {(this.effectiveCDR / 1000).toFixed(1)}
                {t({ id: 'shaman.enhancement.s2TierSet.statistic.secCdr', message: ' sec ' })}
                <small>
                  {t({ id: 'shaman.enhancement.s2TierSet.statistic.cdr', message: 'CDR' })}
                </small>
              </ItemSetBonus>
            </>
          )}
            ...
            {this.hasVoltaicBlaze
              ? this.buildHoldPanel(
                  ...
                  <div>
                    ...
                    <small>
                      {t({
                        id: 'shaman.enhancement.s2TierSet.statistic.cumulativeDelay',
                        message: 'cumulative delay',
                      })}
                    </small>
                  </div>,
                )
              ...
              ? this.buildHoldPanel(
                  ...
                  this.has4Piece ? (
                    <div>
                      ...
                      <small>
                        {t({
                          id: 'shaman.enhancement.s2TierSet.statistic.wastedCdr',
                          message: 'wasted CDR',
                        })}
                      </small>
                    </div>
