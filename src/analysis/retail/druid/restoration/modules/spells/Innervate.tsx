  ...
  get guideCastBreakdown() {
    const explanation = (
      <p>
        ...
        {t({
          id: 'restoration.innervate.explanation',
          message:
            'regenerates 25% of your maximum mana over 8 seconds. Resto Druids should always cast it on themselves, and use it on cooldown whenever it will not waste mana. A cast is good as long as you never hit maximum mana while the buff is up or in the 5 seconds after it ends.',
        })}
      </p>
    );
    ...
    const data = (
      <div>
        <strong>{t({ id: 'restoration.innervate.per_cast_breakdown', message: 'Per-Cast Breakdown' })}</strong>
        <small>{t({ id: 'restoration.innervate.click_to_expand', message: ' - click to expand' })}</small>
        {this.castTrackers.map((cast, ix) => {
          ...
          const header = (
            <>
              ...
              {cast.castOnSelf ? (
                ...
                <>{t({ id: 'restoration.innervate.cast_on_ally', message: ' (cast on ally)' })}</>
              )}
            </>
          );
          ...
          checklistItems.push({
            label: t({ id: 'restoration.innervate.cast_on_self_label', message: 'Cast on yourself' }),
            result: <PassFailCheckmark pass={cast.castOnSelf} />,
          });
          checklistItems.push({
            label: t({
              id: 'restoration.innervate.no_mana_waste_label',
              message: 'Did not hit max mana during buff or 5s after',
            }),
            result: <PassFailCheckmark pass={noManaWaste} />,
            details: cast.manaCapped ? (
              <>
                ({t({ id: 'restoration.innervate.capped_at', message: 'capped @ ' })}
                {cast.cappedAt !== undefined ? this.owner.formatTimestamp(cast.cappedAt) : '?'})
              </>
            ) : cast.castOnSelf ? (
              <>({formatNumber(cast.manaGained)}{t({ id: 'restoration.innervate.mana_gained', message: ' mana gained' })})</>
            ) : undefined,
          });
