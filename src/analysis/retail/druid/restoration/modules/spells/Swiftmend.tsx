...
class Swiftmend extends Analyzer {
    ...
    switch (outcome) {
      case 'rejuv':
        return (
          <>
            ...
            {t({ id: 'restoration.swiftmend.sotf_buffed', message: ' buffed ' })}
      ...
      case 'overwritten':
        return (
          <>
            <SpellLink spell={TALENTS_DRUID.SOUL_OF_THE_FOREST_RESTORATION_TALENT} />
            {t({ id: 'restoration.swiftmend.sotf_overwritten', message: ' overwritten' })}
          </>
        );
      case 'expired':
        return (
          <>
            <SpellLink spell={TALENTS_DRUID.SOUL_OF_THE_FOREST_RESTORATION_TALENT} />
            {t({ id: 'restoration.swiftmend.sotf_expired', message: ' expired' })}
          </>
        );
      case 'unused':
        return (
          <>
            <SpellLink spell={TALENTS_DRUID.SOUL_OF_THE_FOREST_RESTORATION_TALENT} />
            {t({ id: 'restoration.swiftmend.sotf_unused', message: ' unused at fight end' })}
          </>
        );
      default:
        return (
          <>
            <SpellLink spell={TALENTS_DRUID.SOUL_OF_THE_FOREST_RESTORATION_TALENT} />
            {t({ id: 'restoration.swiftmend.sotf_unresolved', message: ' unresolved' })}
          </>
        );
    }
  }
  ...
  private castSummary(cast: SwiftmendCastRecord): JSX.Element {
    ...
    if (this.hasImplant) {
      if (cast.onLifebloomTarget) {
        parts.push(
          <>
            {t({ id: 'restoration.swiftmend.on_lb_target.a', message: 'on ' })}
            <SpellLink spell={SPELLS.LIFEBLOOM_HOT_HEAL} />
            {t({ id: 'restoration.swiftmend.on_lb_target.b', message: ' target' })}
          </>,
        );
      } else if (cast.wasTriage) {
        parts.push(<>{t({ id: 'restoration.swiftmend.triage_cast', message: 'triage cast' })}</>);
      } else {
        parts.push(
          <>
            {t({ id: 'restoration.swiftmend.not_on_lb_target.a', message: 'not on ' })}
            <SpellLink spell={SPELLS.LIFEBLOOM_HOT_HEAL} />
            {t({ id: 'restoration.swiftmend.not_on_lb_target.b', message: ' target' })}
          </>,
        );
      }
    } else if (!this.hasVi) {
      if (cast.wasTriage) {
        ...
        parts.push(
          <>
            {t({ id: 'restoration.swiftmend.consumed', message: 'consumed ' })}
            <SpellLink spell={cast.consumedSpell} />
            {remaining}
          </>,
        );
      } else {
        parts.push(
          <>{t({ id: 'restoration.swiftmend.consumed_unknown', message: 'consumed unknown HoT' })}</>,
        );
      }
    }
      ...
      <RoundedPanel>
        {showEfficiency && (
          <>
            <strong>
              ...
              {t({ id: 'restoration.swiftmend.cast_efficiency', message: ' cast efficiency' })}
            </strong>
            <CastEfficiencyBar
        ...
        {this.trackCastAnalysis && this.casts.length > 0 && (
          <div style={{ minWidth: 0, overflow: 'hidden' }}>
            <CastDetail
              title={t({ id: 'restoration.swiftmend.casts_title', message: 'Swiftmend Casts' })}
    ...
    return (
      <>
        <p>
          ...
          {t({
            id: 'restoration.swiftmend.explanation_p1',
            message:
              'is one of your highest priority spells. Cast it on yourself as close to on cooldown as possible',
          })}
          {cooldownSpells.length > 0 ? (
            <>
              {t({ id: 'restoration.swiftmend.explanation_it', message: '. It ' })}
              {cooldownSpells.map((spell, index) => (
                <Fragment key={spell.id}>
                  {index > 0 &&
                    (index === cooldownSpells.length - 1
                      ? cooldownSpells.length === 2
                        ? t({ id: 'restoration.swiftmend.list_and', message: ' and ' })
                        : t({ id: 'restoration.swiftmend.list_and', message: ', and ' })
                      : t({ id: 'restoration.swiftmend.list_comma', message: ', ' }))}
  ...
  private cooldownSpellVerb(spell: { id: number }): string {
    if (spell.id === TALENTS_DRUID.SOUL_OF_THE_FOREST_RESTORATION_TALENT.id) {
      return t({ id: 'restoration.swiftmend.verb_gives', message: 'gives you' });
    }
    if (spell.id === TALENTS_DRUID.GROVE_GUARDIANS_TALENT.id) {
      return t({ id: 'restoration.swiftmend.verb_summons', message: 'summons' });
    }
    if (spell.id === TALENTS_DRUID.EVERBLOOM_3_RESTORATION_TALENT.id) {
      return t({
        id: 'restoration.swiftmend.verb_blooms',
        message: 'causes 3 Lifebloom blooms via',
      });
    }
    return t({ id: 'restoration.swiftmend.verb_empowers', message: 'empowers' });
  }
  ...
  private renderTargetingGuidance(): JSX.Element | null {
    if (this.hasImplant) {
      return (
        <p>
          {t({ id: 'restoration.swiftmend.implant_p1', message: "If you're playing " })}
          <SpellLink spell={TALENTS_DRUID.IMPLANT_TALENT} />
          {t({
            id: 'restoration.swiftmend.implant_p2',
            message: ', almost every Swiftmend should go onto your ',
          })}
          <SpellLink spell={SPELLS.LIFEBLOOM_HOT_HEAL} />
          {t({
            id: 'restoration.swiftmend.implant_p3',
            message: ' target (usually yourself). This grows a ',
          })}
          <SpellLink spell={SPELLS.SYMBIOTIC_BLOOMS_WILDSTALKER} />
          {t({
            id: 'restoration.swiftmend.implant_p4',
            message:
              ", which increases healing received, and therefore Everbloom's splash healing. The only exception is emergency triage to save a player who would die otherwise.",
          })}
        </p>
      );
    }
    ...
    if (this.hasVi) {
      return (
        <p>
          {t({ id: 'restoration.swiftmend.vi_p1', message: 'With ' })}
          <SpellLink spell={TALENTS_DRUID.VERDANT_INFUSION_TALENT} />
          {t({
            id: 'restoration.swiftmend.vi_p2',
            message:
              ', Swiftmend does not consume a HoT, so you do not need to worry about which HoT is on the target.',
          })}
        </p>
      );
    }
    ...
    return (
      <>
        <p>
          {t({
            id: 'restoration.swiftmend.consume_p1',
            message:
              "Swiftmend removes a HoT on the target. The direct heal gains a portion of that HoT's remaining healing, but losing the HoT still costs throughput, so choose consumes carefully. The game removes HoTs in the order Regrowth, Wild Growth, then Rejuvenation. Try to consume in this priority instead:",
          })}
        </p>
        <ul style={{ marginBottom: '1em' }}>
          <li>
            <SpellLink spell={SPELLS.WILD_GROWTH} />
            {t({ id: 'restoration.swiftmend.consume_li1', message: ' or a low duration HoT (' })}
            &lt;{LOW_HOT_THRESHOLD_MS / 1000}
            {t({ id: 'restoration.swiftmend.consume_li1_b', message: 's) ' })}
            {this.perfBadge(QualitativePerformance.Good)}
          </li>
          <li>
            {t({ id: 'restoration.swiftmend.consume_full_duration', message: 'Full duration ' })}
            <SpellLink spell={SPELLS.REGROWTH} /> {this.perfBadge(QualitativePerformance.Ok)}
          </li>
          <li>
            {t({ id: 'restoration.swiftmend.consume_full_duration', message: 'Full duration ' })}
            <SpellLink spell={SPELLS.REJUVENATION} /> {this.perfBadge(QualitativePerformance.Fail)}
          </li>
        </ul>
        <p>
          {t({ id: 'restoration.swiftmend.triage_p1', message: 'Using it as a triage heal (≤' })}
          {CONSUME_TRIAGE_THRESHOLD * 100}
          {t({
            id: 'restoration.swiftmend.triage_p2',
            message: '% HP) is always fine, regardless of what gets consumed.',
          })}
        </p>
      </>
    );
  ...
  private renderSotfGuidance(): JSX.Element | null {
    ...
    return (
      <p>
        {t({ id: 'restoration.swiftmend.sotf_p1', message: 'Every Swiftmend also grants ' })}
        <SpellLink spell={TALENTS_DRUID.SOUL_OF_THE_FOREST_RESTORATION_TALENT} />
        {t({
          id: 'restoration.swiftmend.sotf_p2',
          message: '. Make sure you spend the proc before your next Swiftmend. ',
        })}
        <SpellLink spell={SPELLS.REJUVENATION} />
        {t({ id: 'restoration.swiftmend.sotf_p3', message: ' is the default spender' })}
        {this.hasAbundance ? (
          <>
            {t({ id: 'restoration.swiftmend.sotf_p4', message: ', and the cheapest way to keep ' })}
            <SpellLink spell={TALENTS_DRUID.ABUNDANCE_TALENT} />
            {t({ id: 'restoration.swiftmend.sotf_p5', message: ' active' })}
          </>
        ) : null}
        {t({ id: 'restoration.swiftmend.sotf_p6', message: '. ' })}
        <SpellLink spell={SPELLS.REGROWTH} />
        {t({
          id: 'restoration.swiftmend.sotf_p7',
          message: ' is also a fine choice if the healing is needed and mana allows.',
        })}
      </p>
    );
  }
    ...
    return (
      <Statistic
        ...
        tooltip={
          <>
            {t({
              id: 'restoration.swiftmend.stat_tooltip_p1',
              message: "Estimated healing from Swiftmend's bonus of ",
            })}
            {(CONSUMED_HOT_BONUS_MULTIPLIER * 100).toFixed(0)}
            {t({
              id: 'restoration.swiftmend.stat_tooltip_p2',
              message:
                "% of the consumed HoT's remaining healing. Remaining HoT healing is estimated from duration left × recent tick size.",
            })}
            <br />
            <br />
            {t({ id: 'restoration.swiftmend.stat_tooltip_bonus', message: 'Estimated bonus: ' })}
      ...
      >
        <BoringSpellValueText spell={SPELLS.SWIFTMEND}>
          ...
          <br />
          <small>{t({ id: 'restoration.swiftmend.stat_from_bonus', message: 'from consumed HoT bonus' })}</small>
