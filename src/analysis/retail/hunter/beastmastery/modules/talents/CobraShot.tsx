  ...
  onCobraShotCast(event: CastEvent) {
    ...
    if (resource.amount < COBRA_SHOT_FOCUS_THRESHOLD_TO_WAIT) {
      addInefficientCastReason(
        event,
        defineMessage({
          id: 'hunter.beastmastery.cobraShot.inefficientCastFocus',
          message:
            "Cobra Shot cast while Kill Command's remaining cooldown was under {cdr}s and you were not close to capping focus as you only had {focus} focus.",
          values: {
            cdr: (this.cobraShotCDR / 1000).toFixed(1),
      ...
      <>
        ...
        <Statistic
          ...
          tooltip={
            <>
              {t({
                id: 'hunter.beastmastery.cobraShot.barbedShotPriorityTooltip',
                message:
                  "Barbed Shot should always be prioritized over Cobra Shot - there's no scenario where casting Cobra Shot instead of an available Barbed Shot is correct.",
              })}
            </>
          }
        >
          <BoringSpellValueText spell={TALENTS.COBRA_SHOT_TALENT}>
            <>
              ...
              <small>
                {t({
                  id: 'hunter.beastmastery.cobraShot.barbedShotAvailable',
                  message: 'Cobra Shots cast with Barbed Shot available',
                })}
              </small>
            </>
          </BoringSpellValueText>
