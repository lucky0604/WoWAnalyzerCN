...
function ResourceUsageSection({ info, modules }: GuideProps<typeof CombatLogParser>) {
  ...
  return (
    <Section
        title={t({ id: 'demonhunter.havoc.sections.core.title', message: 'Core' })}
      >
      <SubSection
      ...
      <SubSection
        title={t({
          id: 'demonhunter.havoc.sections.core.activeTime.title',
          message: 'Active Time',
        })}
      >
        <p>
...
function CooldownSection({ modules, info }: GuideProps<typeof CombatLogParser>) {
  return (
    <Section
        title={t({ id: 'demonhunter.havoc.sections.cooldowns.title', message: 'Cooldowns' })}
      >
...
function RotationSection({ modules, info }: GuideProps<typeof CombatLogParser>) {
  return (
    <Section
        title={t({ id: 'demonhunter.havoc.sections.rotation.title', message: 'Rotation' })}
      >
      <HideExplanationsToggle id="hide-explanations-rotations" />
      <HideGoodCastsToggle id="hide-good-casts-rotations" />
      <p>
        <Trans id="demonhunter.havoc.sections.rotation.summary">
...
function DefensivesSection({ modules }: GuideProps<typeof CombatLogParser>) {
  ...
  return (
    <Section
      ...
      <p>
        {t({
          id: 'demonhunter.havoc.sections.defensives.blur.questions',
          message: 'When reviewing your Blur usage, focus on two questions:',
        })}
      </p>
      <ol>
        <li>
          {t({
            id: 'demonhunter.havoc.sections.defensives.blur.question1',
            message: 'Did Blur cover dangerous spikes or other high-pressure damage windows?',
          })}
          <p>
            <small>
        ...
        <li>
          {t({
            id: 'demonhunter.havoc.sections.defensives.blur.question2',
            message: 'Was Blur used often enough across the fight, or was it held long enough to lose value?',
          })}
          <p>
            <small>
              {t({
                id: 'demonhunter.havoc.sections.defensives.blur.question2.explanation',
                message:
                  'The cooldown timeline below shows whether casts were timed around threatening damage and whether long gaps may have cost you additional uses.',
              })}
            </small>
          </p>
        </li>
      ...
      <SubSection
        title={t({
          id: 'demonhunter.havoc.sections.defensives.damageTaken.title',
